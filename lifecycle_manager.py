#!/usr/bin/env python3
"""
NetworkReview Plugin Lifecycle Manager (New Architecture)

This script handles install/update/delete operations for the NetworkReview plugin
...
try:
    ...
    try:
        ...
        raise ImportError("NetworkReview plugin requires the new architecture BaseLifecycleManager")


class NetworkReviewLifecycleManager(BaseLifecycleManager):
    """Lifecycle manager for NetworkReview plugin using new architecture"""
    ...
    def __init__(self, plugins_base_dir: str = None):
        ...
        self.plugin_data = {
            ...
            "plugin_slug": "BrainDriveNetworkReview",
            # Update tracking fields (matching plugin model)
            "source_type": "github",
            "source_url": "https://github.com/DJJones66/NetworkReview",
            "update_check_url": "https://api.github.com/repos/DJJones66/NetworkReview/releases/latest",
        ...
        logger.info(f"NetworkReview: plugins_base_dir - {plugins_base_dir}")
        if plugins_base_dir:
        ...
        logger.info(f"NetworkReview: shared_path - {shared_path}")
        super().__init__(
    ...
    async def _perform_user_installation(self, user_id: str, db: AsyncSession, shared_plugin_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"NetworkReview: User installation completed for {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"NetworkReview: User installation failed for {user_id}: {e}")
    ...
    async def _perform_user_uninstallation(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"NetworkReview: User uninstallation completed for {user_id}")
            return {
                'success': True,
                'plugin_id': plugin_id,
            ...
            logger.error(f"NetworkReview: User uninstallation failed for {user_id}: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _copy_plugin_files_impl(self, user_id: str, target_dir: Path, update: bool = False) -> Dict[str, Any]:
        """
        NetworkReview-specific implementation of file copying.
        ...
        try:
            ...
            logger.info(f"NetworkReview: Copied {len(copied_files)} files/directories to {target_dir}")
            return {'success': True, 'copied_files': copied_files}
            
        except Exception as e:
            logger.error(f"NetworkReview: Error copying plugin files: {e}")
            return {'success': False, 'error': str(e)}
    
    async def _validate_installation_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        NetworkReview-specific validation logic.
        This method is called by the base class during installation.
        """
        try:
            # Check for NetworkReview-specific required files
            ...
            if missing_files:
                return {
                    'valid': False,
                    'error': f"NetworkReview: Missing required files: {', '.join(missing_files)}"
                }
            ...
            try:
                ...
                for field in required_fields:
                    if field not in package_data:
                        return {
                            'valid': False,
                            'error': f'NetworkReview: package.json missing required field: {field}'
                        }
                        
            except (json.JSONDecodeError, FileNotFoundError) as e:
                return {
                    'valid': False,
                    'error': f'NetworkReview: Invalid or missing package.json: {e}'
                }
            ...
            if bundle_path.stat().st_size == 0:
                return {
                    'valid': False,
                    'error': 'NetworkReview: Bundle file (remoteEntry.js) is empty'
                }
            
            logger.info(f"NetworkReview: Installation validation passed for user {user_id}")
            return {'valid': True}
            
        except Exception as e:
            logger.error(f"NetworkReview: Error validating installation: {e}")
            return {'valid': False, 'error': str(e)}
    
    async def _get_plugin_health_impl(self, user_id: str, plugin_dir: Path) -> Dict[str, Any]:
        """
        NetworkReview-specific health check logic.
        ...
        try:
            ...
            logger.error(f"NetworkReview: Error checking plugin health: {e}")
            return {
                'healthy': False,
                'details': {'error': str(e)}
            }
    ...
    async def _check_existing_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            plugin_slug = self.plugin_data['plugin_slug']
            logger.info(f"NetworkReview: Checking for existing plugin - user_id: {user_id}, plugin_slug: {plugin_slug}")
            
            # First test database connectivity
            test_query = text("SELECT COUNT(*) as count FROM plugin")
            test_result = await db.execute(test_query)
            test_row = test_result.fetchone()
            logger.info(f"NetworkReview: Database connectivity test - total plugins: {test_row.count}")
            ...
            logger.info(f"NetworkReview: Executing query with params: {query_params}")
            
            result = await db.execute(plugin_query, query_params)
            
            plugin_row = result.fetchone()
            logger.info(f"NetworkReview: Query result: {plugin_row}")
            if plugin_row:
                logger.info(f"NetworkReview: Found existing plugin - id: {plugin_row.id}, name: {plugin_row.name}")
                return {
                    'exists': True,
                ...
                logger.warning(f"NetworkReview: No plugin found for user_id: {user_id}, plugin_slug: {plugin_slug}")
                ...
                if debug_rows:
                    logger.info(f"NetworkReview: User has {len(debug_rows)} other plugins:")
                    for row in debug_rows:
                        logger.info(f"  - {row.plugin_slug} (id: {row.id})")
                else:
                    logger.info(f"NetworkReview: User has no plugins installed")
                
                return {'exists': False}
                
        except Exception as e:
            logger.error(f"NetworkReview: Error checking existing plugin: {e}")
    ...
    async def _create_database_records(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"NetworkReview: Creating database records - user_id: {user_id}, plugin_slug: {plugin_slug}, plugin_id: {plugin_id}")
            ...
            await db.commit()
            logger.info(f"NetworkReview: Database transaction committed successfully")
            ...
            if verify_row:
                logger.info(f"NetworkReview: Successfully created and verified database records for plugin {plugin_id} with {len(modules_created)} modules")
            else:
                logger.error(f"NetworkReview: Plugin creation appeared to succeed but verification failed for plugin_id: {plugin_id}")
    ...
    async def _export_user_data(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"NetworkReview: Exported user data for {user_id}")
            return {'success': True, 'user_data': user_data}
            
        except Exception as e:
            logger.error(f"NetworkReview: Error exporting user data: {e}")
            return {'success': False, 'error': str(e)}
    ...
    async def _import_user_data(self, user_id: str, db: AsyncSession, user_data: Dict[str, Any]):
        ...
        try:
            if not user_data:
                logger.info(f"NetworkReview: No user data to import for {user_id}")
                return
            ...
            logger.info(f"NetworkReview: Imported user data for {user_id}")
            
        except Exception as e:
            logger.error(f"NetworkReview: Error importing user data: {e}")
            raise
    ...
    async def install_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Install NetworkReview plugin for specific user (compatibility method)"""
        try:
            logger.info(f"NetworkReview: Starting installation for user {user_id}")
            
            # Check if plugin is already installed for this user
            existing_check = await self._check_existing_plugin(user_id, db)
            if existing_check['exists']:
                logger.warning(f"NetworkReview: Plugin already installed for user {user_id}")
                return {
                    'success': False,
            ...
            logger.info(f"NetworkReview: Created shared directory: {shared_path}")

            # Copy plugin files to the shared directory first
            copy_result = await self._copy_plugin_files_impl(user_id, shared_path)
            if not copy_result['success']:
                logger.error(f"NetworkReview: File copying failed: {copy_result.get('error')}")
                return copy_result

            logger.info(f"NetworkReview: Files copied successfully, proceeding with database installation")
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    if not verify_check['exists']:
                        logger.error(f"NetworkReview: Installation appeared successful but verification failed")
                        return {'success': False, 'error': 'Installation verification failed'}
                    
                    logger.info(f"NetworkReview: Installation verified successfully for user {user_id}")
                    result.update({
                        'plugin_slug': self.plugin_data['plugin_slug'],
                        'plugin_name': self.plugin_data['name']
                    })
                else:
                    logger.error(f"NetworkReview: Database installation failed: {result.get('error')}")
                
                return result
                
            except Exception as db_error:
                logger.error(f"NetworkReview: Database operation failed: {db_error}")
            ...
            logger.error(f"NetworkReview: Install plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    
    async def delete_plugin(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Delete NetworkReview plugin for user (compatibility method)"""
        try:
            logger.info(f"NetworkReview: Starting deletion for user {user_id}")
            ...
            if result.get('success'):
                logger.info(f"NetworkReview: Successfully deleted plugin for user {user_id}")
            else:
                logger.error(f"NetworkReview: Deletion failed: {result.get('error')}")
            
            return result
        except Exception as e:
            logger.error(f"NetworkReview: Delete plugin failed: {e}")
            return {'success': False, 'error': str(e)}
    
    async def get_plugin_status(self, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """Get current status of NetworkReview plugin installation (compatibility method)"""
        try:
            existing_check = await self._check_existing_plugin(user_id, db)
            if not existing_check['exists']:
            ...
            logger.error(f"NetworkReview: Error checking plugin status: {e}")
            return {'exists': False, 'status': 'error', 'error': str(e)}
    
    async def update_plugin(self, user_id: str, db: AsyncSession, new_version_manager: 'NetworkReviewLifecycleManager') -> Dict[str, Any]:
        """Update NetworkReview plugin for user (compatibility method)"""
        try:
            ...
            logger.info(f"NetworkReview: Plugin updated successfully for user {user_id}")
            return {
                'success': True,
            ...
            logger.error(f"NetworkReview: Plugin update failed for user {user_id}: {e}")
            return {'success': False, 'error': str(e)}
...
async def install_plugin(user_id: str, db: AsyncSession, plugins_base_dir: str = None) -> Dict[str, Any]:
    manager = NetworkReviewLifecycleManager(plugins_base_dir)
...
async def update_plugin(user_id: str, db: AsyncSession, new_version_manager: 'NetworkReviewLifecycleManager', plugins_base_dir: str = None) -> Dict[str, Any]:
    old_manager = NetworkReviewLifecycleManager(plugins_base_dir)
...
if __name__ == "__main__":
    ...
    async def main():
        print("NetworkReview Plugin Lifecycle Manager - Test Mode")
        print("=" * 50)
        
        # Test manager initialization
        manager = NetworkReviewLifecycleManager()
